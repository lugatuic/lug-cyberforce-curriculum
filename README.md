================================================================================
# CyberForce 2026 Curriculum & Training

**Welcome!** Whether you're competing, helping, or just curious — start here.

---

## 🎯 Quick Links (Start Here!)

### **For Competitors**
- [What is CyberForce?](docs/cyberforce-101.md) — overview & competition structure
- [Competition Timeline](docs/competition-timeline.md) — dates, phases, what happens when
- [Official CyberForce 101 Library](https://cyberforce.energy.gov/cyberforce-101-library/) — read this
- [2024 Anomalies](https://drive.google.com/drive/folders/1CMDQ8fPsviGhkE3nmA0YcQ7K8DPiedaM?usp=sharing) — study what you'll face
- [Team Structure & Roles](team-formation/role-descriptions.md) — Green, Vuln Hunters, Monitors
- [Technical Prep Guide](resources/technical-prep/) — Linux, Windows, Open PLC, PHP, networking

### **For Event Leaders**
- [Welcome Event Playbook](events/welcome-event/) — agenda, talking points, veteran scripts
- [C-Suite Briefing Prep Guide](events/c-suite-prep/) — practice sessions, recording tips
- [Info Session Guide](events/info-session/) — skill assessment, confidence building
- [March Madness Planning](events/march-madness/) — red-vs-blue competition guide

### **For Admins/Next Year's Leads**
- [Handoff Guide](HANDOFF.md) — **Read this first** if you're taking over
- [Admin Meeting Notes](admin/admin-meeting-notes.md) — decisions made, timeline
- [Timeline for Admins](admin/timeline-internal.md) — deadlines, checkpoints
- [Common Pitfalls](docs/common-pitfalls.md) — what went wrong + how to fix

### **For Alumni/Mentors**
- [Mentoring Guide](team-formation/mentor-matching.md) — how to help a team
- [Red Team Guide](events/march-madness/red-team-guide.md) — help run the competition
- [Archive: Lessons from Veterans](archival/red-team-knowledge/) — Michael's insights

---

## 📚 What's in This Repo?

```
/docs/                 → General knowledge (what is CF, timeline, scoring)
/resources/            → Learning materials (Linux, Windows, Open PLC, tools)
/events/               → Event playbooks (welcome, C-Suite, info session, March Madness)
/team-formation/       → Team building (roles, balancing, mentors)
/competition-prep/     → Week-by-week curriculum (Sept–Nov)
/announcements/        → Discord & email templates
/scorecards-feedback/  → Past competition results & lessons learned
/archival/             → Red team knowledge, anomaly solutions, team reports
/admin/                → Internal notes, budget, contacts
/templates/            → Copy-paste templates for future years
```

---

## 🎯 How to Use This Repo

**If you're competing:**
1. Read [CyberForce 101](docs/cyberforce-101.md)
2. Check [Competition Timeline](docs/competition-timeline.md)
3. Study [2024 Anomalies](https://drive.google.com/drive/folders/1CMDQ8fPsviGhkE3nmA0YcQ7K8DPiedaM?usp=sharing)
4. Explore [Technical Prep](resources/technical-prep/) based on your role
5. Attend workshops & events listed in calendar

**If you're running an event:**
1. Go to `/events/[event-name]/`
2. Read the agenda + guides
3. Copy the templates
4. Run the event
5. Add notes to [lessons-learned](archival/red-team-knowledge/) for next year

**If you're taking over next year:**
1. **Read [HANDOFF.md](HANDOFF.md) first**
2. Check [Timeline for Admins](admin/timeline-internal.md)
3. Review [Common Pitfalls](docs/common-pitfalls.md)
4. Skim [Admin Meeting Notes](admin/admin-meeting-notes.md)
5. Check GitHub Issues for unfinished work
6. Ask the previous leads (AJ, Michael, etc.) questions

---

## 📅 2026 Main Event Timeline (Quick Reference)

| When | What | Where |
|------|------|-------|
| Early July | Info Session (CF veterans experience) | Online, TBA |
| July 15 @ 6 PM | Official CyberForce 101 Webinar | Online |
| Mid-July | Registration opens | CyberForce.energy.gov |
| September | Welcome/Intro event (veterans talk) | In-person |
| October | C-Suite briefing prep sessions | In-person |
| November | Live competition | Near Chicago |
| March 2027 | March Madness (red-vs-blue event) | In-person, TBA |

*[Detailed timeline in [Competition Timeline](docs/competition-timeline.md)]*

---

## ❓ Questions?

**General questions about CyberForce?**
→ Read [CyberForce 101](docs/cyberforce-101.md) or check [FAQ (coming soon)]

**Technical questions?**
→ Check [Technical Prep](resources/technical-prep/)

**Questions about your role/team?**
→ DM @AJ or check [Team Structure](team-formation/role-descriptions.md)

**This repo has an issue?**
→ Create a GitHub Issue or message @AJ

---

## 🤝 Contributing

Want to add something? See [CONTRIBUTING.md](CONTRIBUTING.md) for:
- How to add new materials
- File naming conventions
- When to create vs. update files
- Who to ask for feedback

**Key rule:** Add "Last updated" dates to everything so the next lead knows what's current.

---

## 📋 Repo Status

**Last updated:** June 2026  
**Maintained by:** AJ & LUG CTF Group Admin team  
**For questions:** See above or check [CONTRIBUTING.md](CONTRIBUTING.md)

---

## 🏆 What This Repo Is For

This repo is the **single source of truth** for CyberForce curriculum, events, and knowledge. It exists so that:

✅ Competitors have clear learning paths  
✅ Event leaders have playbooks to run events  
✅ Admins have timelines & checklists  
✅ Institutional knowledge doesn't disappear when people graduate  
✅ Next year's leads don't reinvent everything  

Make it better each year. Leave notes for the next person. Document lessons learned.

---

**Ready to get started?** Pick a link above and dig in! 🚀

================================================================================
